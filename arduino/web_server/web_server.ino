...
const char index[] PROGMEM =
    ...
    "<form action=\"/btn1\" method=\"POST\"><input type=\"submit\" name=\"value\" value=\"control\" /></form>";
...
void loop() {
  ...
  if (pos) {
    ...
    if (strncmp("GET / ", data, 6) == 0) {
    ...
    } else if (strncmp("POST /btn1", data, 10) == 0) {
      Serial.println(data);
      ...
      bfill.emit_p(PSTR("$F\r\n$F\r\n"),ok_header, index);
    } else if (strncmp("POST /operator/led_on", data, 5) == 0) {
      Serial.println(data);
      bfill.emit_p(ok_header);
    } else if (strncmp("POST /operator/", data, 15) == 0) {
      Serial.println(data);
      bfill.emit_p(ok_header);
    } else {
      bfill.emit_p(PSTR(
